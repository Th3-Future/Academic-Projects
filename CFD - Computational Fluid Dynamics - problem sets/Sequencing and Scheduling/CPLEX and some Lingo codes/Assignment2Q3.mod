 ...
 * OPL 20.1.0.0 Model
 * Author: ezeor
 * Creation Date: 1 Jul 2021 at 17:26:14
 *********************************************/
 
/*We exclude worker 6 from this optimization since we know that packaging(28sec) and
  checkout(10sec) must be done last, which both sum up to 38sec, which is not optimal
  so we consider only the first 5 workers*/
int workers=5;
int tasks=6;
int min2hr=60*60;
range n=1..tasks;
...
int add[m]=[18,0,0,0,28]; /*compulsory added task that must be performed in worker 1 and 5*/
